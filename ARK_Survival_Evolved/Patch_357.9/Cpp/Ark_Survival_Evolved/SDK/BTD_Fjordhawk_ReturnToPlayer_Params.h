#pragma once

/**
 * Name: Ark_Survival_Evolved
 * Version: Patch_357.9
 */

#ifdef _MSC_VER
	#pragma pack(push, 0x01)
#endif

namespace CG
{
	// --------------------------------------------------
	// # Structs
	// --------------------------------------------------
	/**
	 * Function BTD_Fjordhawk_ReturnToPlayer.BTD_Fjordhawk_ReturnToPlayer_C.ReceiveConditionCheck
	 */
	struct UBTD_Fjordhawk_ReturnToPlayer_C_ReceiveConditionCheck_Params
	{
	public:
		class AActor*                                              OwnerActor;                                              // 0x0000(0x0008)  (Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	};

	/**
	 * Function BTD_Fjordhawk_ReturnToPlayer.BTD_Fjordhawk_ReturnToPlayer_C.ExecuteUbergraph_BTD_Fjordhawk_ReturnToPlayer
	 */
	struct UBTD_Fjordhawk_ReturnToPlayer_C_ExecuteUbergraph_BTD_Fjordhawk_ReturnToPlayer_Params
	{
	public:
		int32_t                                                    EntryPoint;                                              // 0x0000(0x0004)  (Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	};

}

#ifdef _MSC_VER
	#pragma pack(pop)
#endif
